...
func TestSetup(t *testing.T) {
    ...
    // Check the balance
    lamports, ok, _ := svm.Balance(payer)
    if !ok {
...
func TestTransfer(t *testing.T) {
    ...
    lamports, _, _ := svm.Balance(recipient)
...
```go
sim, _ := svm.SimulateLegacyTransaction(txBytes)
defer sim.Close()
...
```go
svm.SetSigverify(false)         // accept unsigned / badly-signed txs
svm.SetBlockhashCheck(false)    // skip recent-blockhash enforcement
svm.SetTransactionHistory(0)    // 0 disables dedup; any N caps history
svm.SetLogBytesLimit(-1)        // negative = unlimited
svm.SetLamports(1 << 40)        // default lamports for new accounts
svm.SetSysvars()                // reset sysvars to defaults
svm.SetBuiltins()               // reload built-in programs
svm.SetDefaultPrograms()        // reload SPL Token, Memo, etc.
svm.SetPrecompiles()            // enable ed25519 / secp256k1 precompiles
svm.WithNativeMints()           // seed wrapped-SOL mint
```

Each call returns an `error`; ignore it if you do not need to react to failure.
