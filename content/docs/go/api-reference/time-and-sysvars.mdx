...
for _, size := range []int{0, 100, 1024} {
    min, _ := svm.MinimumBalanceForRentExemption(size)
...
```go
last, _ := svm.LastRestartSlot()
_ = svm.SetLastRestartSlot(last + 1)
```
...
| Method                            | Description                            |
| --------------------------------- | -------------------------------------- |
| `litesvm.NewSlotHistory()`        | Empty handle                           |
| `sh.Add(slot uint64)`             | Record a slot                          |
| `sh.Check(slot uint64) SlotHistoryCheck` | `Found` / `NotFound` / `TooOld`  |
...
type StakeHistoryItem struct {
    // See litesvm-go for the full struct.
}
