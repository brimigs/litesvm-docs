...
func TestProgramEndToEnd(t *testing.T) {
    ...
    // Wide-open config: faster tests, less plumbing.
    _ = svm.SetSigverify(false)
    _ = svm.SetBlockhashCheck(false)
    _ = svm.SetSysvars()
    _ = svm.SetBuiltins()
    ...
    rent, _ := svm.MinimumBalanceForRentExemption(len(dataPayload))
...
_ = svm.SetDefaultPrograms() // SPL Token, Memo, etc.
_ = svm.WithNativeMints()    // wrapped-SOL mint
```
