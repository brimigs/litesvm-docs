...
description: Learn how call an instruction from your program in your test
---
...
After deploying a program to LiteSVM, you need to execute instructions to interact with it. LiteSVM provides a simple API for creating, sending, and simulating transactions that is fully compatible with Solana's transaction model.
...
let instruction = Instruction {
    ...
    accounts: vec![                     // Accounts the program needs
        ...
        AccountMeta::new_readonly(readonly_pubkey, false), // Read-only
...
let (create_lut_ix, lut_address) = create_lookup_table(
    payer.pubkey(),
    payer.pubkey(),
    0
);
...
let extend_ix = extend_lookup_table(
    ...
    payer.pubkey(),
    Some(payer.pubkey()),
    vec![account_to_lookup],
);

// Execute lookup table setup
let setup_tx = Transaction::new(
    &[&payer],
    Message::new(&[create_lut_ix, extend_ix], Some(&payer.pubkey())),
    svm.latest_blockhash(),
);
svm.send_transaction(setup_tx).unwrap();

// Advance slot (lookup tables can't be used in same slot)
svm.warp_to_slot(1);

// Use lookup table in V0 message
let alta = AddressLookupTableAccount {
    key: lut_address,
    addresses: vec![account_to_lookup],
};

let v0_msg = MessageV0::try_compile(
    &payer.pubkey(),
    &[instruction],
    &[alta],  // Lookup tables
    svm.latest_blockhash(),
...
let v0_tx = VersionedTransaction::try_new(
    VersionedMessage::V0(v0_msg),
    &[&payer]
).unwrap();

svm.send_transaction(v0_tx).unwrap();
```
...
When using SPL Token (included by default):
