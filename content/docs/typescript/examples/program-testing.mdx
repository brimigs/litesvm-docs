...
Testing a custom Solana program means loading its compiled `.so` file into the SVM, setting up the accounts it expects, and sending instructions against it. LiteSVM handles all of this in-process — no validator needed. We recommend generating program clients with the [Codama JS Renderer](https://github.com/codama-idl/renderers-js) so you can have access to a compatible Kit Plugin.
...
const client = (await createClient().use(myProgramPlugin()));
