...
Overwrite the clock sysvar. Get the clock, mutate its fields, then write it back. Use this when you need precise control over multiple clock fields — for example, setting the epoch or unix timestamp, which `warpToSlot` does not update.
...
<Callout type="info">
  `warpToSlot` only updates `slot`. Use `setClock` when your program reads `epoch` or `unix_timestamp` from the Clock sysvar.
</Callout>
