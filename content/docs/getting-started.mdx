...
Inside your program directory, create a folder for `tests`.

```bash
mkdir tests
```

Now `src` and `tests` should be at the same level in your directory structure.
...
Lets write your first LiteSVM test.
...
Now lets create your second LiteSVM test.
...
<Callout type="success">
  **That's it!** Now you've successfully make litesvm tests that create and fund accounts and executes transactions. 
</Callout>
</Step>
...
- **[References →](/docs/complete-examples)** Learn advanced features and custom configurations
